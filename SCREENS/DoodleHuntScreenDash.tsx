...
export default function DoodleHuntDashScreen() {
  ...
  // Check for active dash game when component mounts
  useEffect(() => {
    initializeDashGame();
  }, []);

  // Reload guesses when screen comes into focus
  useFocusEffect(
    useCallback(() => {
      if (gameId) {
        console.log('Screen focused, reloading guesses for game:', gameId);
        loadGameGuesses(gameId);
      }
    }, [gameId])
  );

  // Initialize dash game (get or create active game)
  const initializeDashGame = async () => {
    try {
      setIsLoadingWord(true);
      setWordError(null);

      // Get or create dash game using the new function
      const { data: dashGame, error } = await supabase.rpc('get_or_create_dash_game', {
        target_difficulty: 'easy' // Default to easy for now
      });

      if (error) {
        console.error('Error getting dash game:', error);
        setWordError('Failed to load game');
        return;
      }

      if (dashGame && dashGame.length > 0) {
        const game = dashGame[0];
        setGameId(game.game_id);
        setCurrentLevel(game.current_level);
        setSecretWord(game.current_word);
                ...
                },
                {
                  ...
                  onPress: async () => {
                    ...
                    if (tokenUsed) {
                      ...
                      if (game.game_id) {
                        try {
                          const { error } = await supabase.rpc('reset_dash_guesses');
                          if (error) {
        ...
        setWordError('Failed to create game');
      }
    } catch (error) {
      console.error('Error in initializeDashGame:', error);
      setWordError('Failed to initialize game');
    } finally {
  ...
  // Note: Dash mode doesn't need daily word subscriptions
  ...
  const fetchRandomWord = async () => {
    try {
      ...
      // Note: Dash mode uses get_or_create_dash_game which handles existing games automatically
  ...
  const loadGameGuesses = async (gameUuid: string) => {
    try {
      console.log('🔍 Loading guesses for game:', gameUuid);
      
      // Get current level from the game
      ...
      const currentLevel = gameData.current_level;
      console.log('📊 Current level from database:', currentLevel);
      ...
      console.log('🔍 Guesses query result:', { 
        gameId: gameUuid, 
        level: currentLevel, 
        guesses: guesses, 
        error 
      ...
      if (guesses && guesses.length > 0) {
        console.log('✅ Found guesses, setting state:', guesses.length, 'guesses');
        ...
        console.log('📝 Set previous attempts:', previousAttemptsData);
  ...
  const addGuessToDatabase = async (guess: string, score: number, hint: string) => {
    ...
    try {
      console.log('💾 Adding guess to database:', {
        gameId,
        currentLevel,
        attempts: attempts + 1,
      ...
      const { data: guessId, error } = await supabase.rpc('add_dash_game_guess', {
        game_uuid: gameId,
  ...
  const fetchUpdatedGuessesLeft = async () => {
    ...
    try {
      const { data: gameData, error } = await supabase
        .from('doodle_hunt_dash_games')
        .select('guesses_left, current_level')
        .eq('id', gameId)
        .single();
      ...
      if (gameData) {
        ...
        setCurrentLevel(gameData.current_level);
  ...
  const unlockHint = async (guessId: string) => {
    ...
    try {
      const { data, error } = await supabase.rpc('unlock_dash_hint_with_token', {
        game_uuid: gameId,
        guess_id: guessId
      });
      ...
      if (data && data.success) {
        // Update the hint_used status in local state with the hint from database
        setPreviousAttempts(prev => prev.map(attempt => 
          attempt.guessId === guessId 
            ? { ...attempt, hintUsed: true, hint: data.hint }
            : attempt
        ));

        // Update user tokens in Redux store (tokens are already deducted by the database function)
        if (userInfo) {
          dispatch(setUserInfo({
            ...userInfo,
        ...
        // Show hint in alert
        Alert.alert('Hint Unlocked!', `💡 ${data.hint}`);
        console.log('Hint unlocked successfully');
        return true;
      } else {
        Alert.alert('Error', 'Failed to unlock hint');
        return false;
      }
    } catch (error) {
  ...
  const advanceToNextLevel = async () => {
    try {
      // Call the advance level function
      const { data: nextLevelData, error } = await supabase.rpc('advance_dash_level');
      ...
      if (nextLevelData && nextLevelData.length > 0) {
        ...
        setCurrentLevel(nextLevel.new_level);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Doodle Hunt Dash</Text>
        {!gameWon && !gameLost && (
          <TouchableOpacity
      ...
      <View style={styles.gameInfo}>
        ...
        {/* Level Display - Absolutely positioned on the right */}
        <View style={styles.levelDisplayTop}>
          <Text style={styles.levelDisplayLabel}>Level {currentLevel}</Text>
        </View>
