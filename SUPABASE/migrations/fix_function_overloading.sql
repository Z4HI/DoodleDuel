-- Fix function overloading issue by dropping the old function first
-- This migration removes the hint functionality and resolves function overloading

-- Remove hint column from guesses table
ALTER TABLE public.guesses 
DROP COLUMN IF EXISTS hint;

-- Drop the old function with hint parameter first
DROP FUNCTION IF EXISTS public.add_doodle_hunt_guess(
  game_uuid UUID,
  guess_num INTEGER,
...
-- Create the new function without hint parameter
